...
due: "2026-09-04"
state: p0
state_label: "인증·허들 추가 9/16"
eyebrow: "기획 05"
description: "프론트가 화면 5장을 만들 수 있는 최소 계약. 9/16부터 구글 로그인과 과잉 사용 방지 허들이 붙었습니다."
---

<div class="callout callout--warn">
  <div class="callout__title">이 문서의 「인증 없음」은 더 이상 사실이 아닙니다 (2026-09-16)</div>
  <p>아래 본문은 09-03 설계입니다. 지금은 <strong>구글 로그인</strong>이 필요하고 과잉 사용 방지 허들이 붙었습니다.
  엔드포인트 경로도 본문과 다릅니다. 정본은 개발 저장소의 <code>docs/spec/api_contract.md</code>입니다.</p>
</div>

## 현행 — 인증과 허들 (2026-09-16)

- **구글 로그인 필수.** 판을 만들고 말하고 제출하는 쓰기 요청은 전부 로그인이 필요합니다. 결과 조회 같은 읽기 요청은 로그인 없이 열립니다
- **판을 만든 사용자 기준으로 사용량을 셉니다.** 하루 판 수 제한과 전역 정원이 이 기준으로 걸립니다

| HTTP | 뜻 | 화면 |
|---|---|---|
| **401** | 로그인 필요 | 로그인 안내 |
| **403** `daily_attempt_limit` | 이 사용자의 오늘 판 수 소진 (사용자당 **하루 5판**) | 「오늘은 여기까지」 |
| **429** | 요청이 너무 잦음 — `Retry-After` 헤더로 대기 시간 안내 | 잠시 후 재시도 |
| **503** `daily_cap` | 오늘 전역 정원 마감 | 「오늘 정원이 마감됐다」 |
| 413 / 422 | 요청이 너무 크거나 입력 상한 초과 (발화·질문은 **200자**) | 입력 오류 |

세부 한도와 배포 설정은 공개 문서에 적지 않고 운영 환경에서 관리합니다.
...
| 버전 | `/api/v1/...` |
| **인증** | ~~없음. 익명 세션 키를 `X-Session-Key` 헤더로 전달~~ → **구글 OAuth + `rd_session` 쿠키** (09-16, 위 「현행」 참고) |
...
| 2026-09-15~16 | **구글 로그인 도입, 익명 진입 폐기** | 판을 만든 사용자(`attempts.user_id`)가 하루 판 수·전역 정원 집계의 기준 |
| 2026-09-16 | 과잉 사용 방지 허들 — 사용자당 하루 판 수 · 전역 정원 · 요청 빈도 · 입력 길이 | 판당 API 비용의 상한을 코드로 건다. Anthropic 콘솔 지출 한도와 함께 방어선 |
